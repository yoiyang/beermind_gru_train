...
if __name__ == "__main__":
    ...
    model = baselineGRU(cfg) # Replace this with model = <your model name>(cfg)
    if cfg['cuda']:
    ...
    # load the model
    model.load_state_dict(torch.load("./model_cache_at2_16000"))
    model.eval()
    ...
    for i in range(0,len(X_test),2000):
        print("At " + str(i) + " - time at: " + str(datetime.datetime.now()))
        if (i+2000 < len(X_test)):
            outputs = generate(model, X_test[i:i+2000], cfg) # Generate the outputs for test data
        else:
            outputs = generate(model, X_test[i:], cfg) # Generate the outputs for test data
        out_fname = "reviews_tau_001_at_" + str(i) + ".txt"
    ...
    for i in range(0,len(X_test),2000):
        if (i+2000 < len(X_test)):
            ...
            outputs = generate(model, X_test[i:], cfg) # Generate the outputs for test data
        out_fname = "reviews_tau_04_at_" + str(i) + ".txt"
        # torch.save(model.state_dict(), "./model_final") # save the model
        save_to_file(outputs, out_fname) # Save the generated outputs to a file
        del outputs

    cfg['gen_temp'] = 0.01
    for i in range(40000,len(X_test),2000):
        if (i+2000 < len(X_test)):
            outputs = generate(model, X_test[i:i+2000], cfg) # Generate the outputs for test data
        else:
            outputs = generate(model, X_test[i:], cfg) # Generate the outputs for test data
        out_fname = "reviews_tau_10_at_" + str(i) + ".txt"
        # torch.save(model.state_dict(), "./model_final") # save the model
        save_to_file(outputs, out_fname) # Save the generated outputs to a file
        del outputs
