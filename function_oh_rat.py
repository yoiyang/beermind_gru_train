...
def process_test_data(data):
    ...
    for line in iter(data.iterrows()):
        ...
        # tensor for rating
        dat[review_num][0][104 + int(line[1]['ratings']) % 5] = 1

        # encoded review with a <SOS> in it
        dat[review_num][0][110:] = char2oh('\x02')
...
def process_train_data(data):
    ...
    dat = np.zeros([len(data),len(data['reviews'][0]),208])
    labels = np.zeros([len(data),len(data['reviews'][0]),1])
    review_num = 0
    for line in iter(data.iterrows()):
...
def generate(model, X_test, cfg):
    ...
    for row_ind in range(len(X_test)):
        ...
        # get the beer style, rating and first character <SOS>
        #print(X_test[row_ind][0])
        inp = np.array([[np.array(X_test[row_ind][0]) ]])

        # let the model generate next characters
        while(count > 0):

            # get the next output from the model
            output = model(torch.from_numpy(inp).float().to(computing_device))
            ...
            reviews[row_ind] += pos2char(pos)

            # create one hot vector of the character
            oh = np.zeros(98)
            oh[pos] = 1

            # update the input vector for the next input
            inp[0][0][-98:] = oh
...
def save_to_file(reviews, fname):
    # Given the list of strings and output file name, save all these reviews to
